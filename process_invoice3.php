    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['save_and_download'])) {
            ...
            if ($result->num_rows > 0) {
                ...
                // Insert new record
                $insert_sql = "INSERT INTO invoice_data (username, from_field, bill_to, invoice_number, invoice_date, terms, invoice_currency_format) VALUES (?, ?, ?, ?, ?, ?, ?)";
                $insert_stmt = $conn->prepare($insert_sql);
                $insert_stmt->bind_param("sssssss", $username, $from, $bill_to, $invoice_number, $invoice_date, $terms, $invoice_currency_format);
